...
# FIXME:translate into wand's way of things:
...
class Engine(BaseEngine):
    ...
    def create_image(self, buffer):
        try:
            img = Image(blob=buffer)
        # except DECOMPRESSION_BOMB_EXCEPTIONS as error:
        except Exception as error:
            logger.warning("[WandEngine] create_image failed: %s", error)
            raise  # FIXME: check wether Wand has such exception or remove this entirely
            # return None
        # self.icc_profile = img.info.get("icc_profile")  # FIXME: !
        # self.exif = img.info.get("exif")  # FIXME: !
        self.original_colorspace = img.colorspace

        self.subsampling = img.options["jpeg:sampling-factor"]
        if not self.subsampling:  # n/a for this file
            self.subsampling = None
        self.qtables = getattr(img, "quantization", None)  # FIXME: :-)
    ...
    def read(self, extension=None, quality=None):  # NOQA
        ...
        if (
            ...
            and self.original_colorspace in ["P", "1"]
            and self.original_colorspace != self.image.mode
        ):
            if self.original_colorspace == "1":
    ...
    def image_data_as_rgb(self, update_image=True):
        ...
        if converted_image.colorspace not in ["rgb", "srgb"]:
            if "s" in converted_image.colorspace:
                converted_image = converted_image.convert("srgb")
            elif converted_image.colorspace == "P":
                # convert() figures out RGB or RGBA based on palette used
